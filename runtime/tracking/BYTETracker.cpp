...
int update_byte_tracker(
   ...
   return tracker->update(objects, count, outputs, output_count);
}
...
int BYTETracker::update(const BtInput *objects, int count, int *outputs, int output_count) {
   ...
   if (count > 0) {
      for (int i = 0; i < count; i++) {
         ...
         STrack strack(STrack::tlbr_to_tlwh(tlbr_), score, objects[i].zig_id);
   ...
   for (const auto &strack : removed_stracks) {
      cout << "Removed track: " << strack.zig_id << endl;
   }

   int write_idx = 0;
   for (int i = 0; i < this->tracked_stracks.size(); i++) {
      if (this->tracked_stracks[i].is_activated) {
         outputs[write_idx] = this->tracked_stracks[i].zig_id;
         write_idx++;
      }
   }
   return write_idx;
